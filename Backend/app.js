...
app.post('/stockAnalysisSearch',function(req,res){
	let nameOfSearchAnalysisID = req.body.stockAnalysisSearch;
		productModel.findOne({productName:nameOfSearchAnalysisID})
		  .then(productFound => {
		  		productQuantity = productFound.productQuantity;
		  		id = productFound.productID;
		  		stockAnalysisModel.findOne({item:id})
		  			.then(productIdFound => {
		  				productSalesPerMonth = productIdFound.total_sales;
		  				productSalesPerDay  = Math.round(productSalesPerMonth/30);
		  				Daysleft = 0;
		  				for(i=1;i<16;i++){
		  				...
		  				res.render('stockAnalysis',{nameOfSearchAnalysisID:nameOfSearchAnalysisID,
		  ...
		  .catch(error => {
		    console.error('Error finding product name before ID:', error);
		    res.redirect("stockAnalysis");
		  });			
})
